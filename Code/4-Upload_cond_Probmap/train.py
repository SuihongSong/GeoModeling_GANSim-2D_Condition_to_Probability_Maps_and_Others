...
def setup_snapshot_image_grid(G, training_set,
    ...
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, label, probimage, _ = training_set.get_minibatch_np(1)
    ...
    return (gw, gh), reals, labels, probimages, latents
...
def train_progressive_gan(
    ...
    resume_run_id           = None, #'/scratch/users/suihong/ProGAN_MultiChannel_Reusults_ConditionedtoMultiConditions_TF/1031-pgan-CondProb-w0.1-new-FS-2gpu/network-snapshot-012160.pkl',         
    ...
    resume_time             = 0): #  15*3600+31*60+1       # seconds, Assumed wallclock time at the beginning. Affects reporting.
    ...
    with tf.name_scope('Inputs'):
        ...
        reals, labels, probimages, _  = training_set.get_minibatch_tf(minibatch_in)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=G_opt, lod = lod_in, training_set=training_set, labels = labels_gpu, prob_images = probimages_gpu, minibatch_size=minibatch_split,**config.G_loss)
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu, prob_images = probimages_gpu, **config.D_loss)
    ...
    grid_size, grid_reals, grid_labels, grid_probimages, grid_latents = setup_snapshot_image_grid(G, training_set, **config.grid)
    sched = TrainingSchedule(total_kimg * 1000, training_set, **config.sched)
    grid_fakes = Gs.run(grid_latents, grid_labels, grid_probimages, minibatch_size=sched.minibatch//config.num_gpus)
